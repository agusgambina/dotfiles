...
for DOTFILE in "$DOTFILES_DIR"/system/shell/.{function,function*,path,env,alias,completion,grep,prompt,custom}; do
  [ -f "$DOTFILE" ] && . "$DOTFILE"
done

if [ "$OS" = "OSX" ]; then
  for DOTFILE in "$DOTFILES_DIR"/system/osx/.{env,alias,function}.osx; do
    [ -f "$DOTFILE" ] && . "$DOTFILE"
  done
fi
